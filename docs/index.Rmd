...
Cross-validation involving splitting the data into different groups, or "folds" (usually 5), training the model on 3/4 of the data and test the resulting model on 1/4 of the data that has been held out.
...
getPerplexity = function(k, fold) {
    ...
    training.model = LDA(bills_dfm[training.dtm, ], k = k) 
...
At first glance, folds appear to have varying trends. In particular, 2 and 5 have a much steeper downward pattern than the others. In folds 1, 3, and 4, leveling off may occur closer to 50 topics.
