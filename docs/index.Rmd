...
getPerplexity = function(k, fold) {
    ...
    training.dtm = which(folding_sets != fold) # rest (4/5) of the data used to train the model (not the fold)
    
    training.model = LDA(bills_dfm[training.dtm, ], k = k) # train LDA model using 4/5 of data
    test.model = LDA(bills_dfm[testing.dtm, ], model = training.model, control = list(estimate.beta = FALSE)) # test on 1/5
    
    perplexity(test.model) # calculate perplexity on the test model (1/5)
}
